...
const buildCoverFocusTransform = (coverRegion, focusProgress) => {
  ...
  const paddedWidth = Math.min(1, width + (isBannerRegion ? 0.02 : isWideRegion ? 0.005 : 0.02));
  const paddedHeight = Math.min(1, height + (isBannerRegion ? 0.03 : 0.02));
  const largeRegionBoost = isBannerRegion ? 1.06 : isWideRegion ? 1.2 : width > 0.38 || height > 0.18 ? 1.08 : 1;
  const horizontalFit = isBannerRegion ? 0.9 / paddedWidth : isWideRegion ? 0.96 / paddedWidth : 0.82 / paddedWidth;
  const verticalFit = isBannerRegion ? 0.82 / paddedHeight : 0.66 / paddedHeight;
  const baseScale = Math.min(horizontalFit, verticalFit);
  const targetScale = Math.max(1, Math.min(3.3, baseScale * largeRegionBoost));
...
export const CoverStage = ({
  ...
  if (currentSegment.segmentType === "intro" && Array.isArray(newspaperCoverStack) && newspaperCoverStack.length > 0) {
    ...
    const stackScale =
      introStack.length >= 5 ? 0.64 : introStack.length === 4 ? 0.7 : introStack.length === 3 ? 0.76 : introStack.length === 2 ? 0.84 : 0.92;
    const stackOffsetX =
      introStack.length >= 5 ? 122 : introStack.length === 4 ? 154 : introStack.length === 3 ? 188 : introStack.length === 2 ? 244 : 0;
